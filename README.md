...
External settlement needs operating gas reserves in the recipient wallet, separate from customer payments: pathUSD on Tempo and native ETH on Base. Run `node scripts/prod-payment-rails-preflight.mjs` before a production payment canary. It checks minimum reserves without moving funds; the guarded canary workflow runs it automatically. The minimums are safety floors, not a guarantee against changing network fees or payment volume.

The production smoke and payment canary use a dedicated buyer key in the repository secret `WALLET_SMOKE_PRIVATE_KEY`. Set the public repository variable `WALLET_SMOKE_ADDRESS` to its derived EVM address; both workflows reject a mismatched key before using it. This key must remain separate from the settlement signer.
