...
const fundingSchema = z.object({ payment_rail: z.literal('ledger'), expected_total: z.number().finite().positive() })
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!input.success) return NextResponse.json({ error: 'Confirm the sandbox ledger total to fund this task.', details: input.error.issues }, { status: 400 })
    if (principal.kind === 'mpp') return NextResponse.json({ error: 'External marketplace payments are unavailable' }, { status: 503 })
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const trade = await createLedgerTrade(tx, listing, principal.userId, feeRecipient, { agentId: principal.agentId })
    ...
    return NextResponse.json({ ok: true, trade: result.trade, workspace_url: `/taskboard/${id}` }, { status: result.created ? 201 : 200 })
  } catch (error) {
