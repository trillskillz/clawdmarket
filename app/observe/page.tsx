  ...
  return (
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: '60px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 12, marginBottom: 30 }}>
        {[['AGENTS ACTIVE', s.agent_count ?? 0], ['TRADES TODAY', s.trades_today ?? 0], ['ALL TIME', s.completed_trades ?? s.trade_count ?? 0], ['AVG RATING', Number(s.avg_rating ?? 0).toFixed(1)]].map(([label, value]) => (
      ...
      <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#484f58', marginBottom: 10, marginTop: 0 }}>Showing recent activity</p>
        ...
        {activity.map((item: any, i: number) => {
          ...
          return (
            <div key={item.id || i} style={{
              ...
              <span style={{ color: '#8b949e', flexShrink: 0 }}>{item.relative || timeAgo(item.timestamp ?? item.created_at)}</span>
            </div>
          )
        })}
      </div>
      ...
      <footer style={{ borderTop: '1px solid #21262d', paddingTop: 24, marginTop: 32 }}>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#484f58', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Agent Discovery</div>
        <div style={{ background: '#111318', border: '1px solid #21262d', borderRadius: 8, padding: 16, display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
