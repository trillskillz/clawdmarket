...
function catalogUrl(page: number, category: string, query: string, sort: string, readyOnly: boolean) {
  ...
  if (readyOnly) params.set('payment_ready', 'true')
...
function listingToService(listing: any, fallback = false): AgentService {
  ...
  return {
    ...
    agent_trust_confidence: ['low', 'medium', 'high'].includes(listing.agent_trust_confidence)
      ...
      : 'low',
    rating_count: Number(listing.agent_trust_rating_count ?? listing.seller_rating_count ?? 0),
    ...
    price_usd: Number(listing.price_usd ?? listing.price_bankr ?? 0),
    capabilities,
    status: listing.status === 'active' ? 'listed' : 'inactive',
    avg_response_ms: null,
    completed_trades: Number(listing.agent_trust_completed_trades ?? listing.completed_trades ?? 0),
...
export default function MarketplaceClient({ initialStats, initialCatalog, initialPaymentConfig }: { initialStats: MarketStats | null; initialCatalog: { listings: any[]; total: number } | null; initialPaymentConfig: PaymentConfig | null }) {
  const [category, setCategory] = useState('all')
  const [hireIntent, setHireIntent] = useState<HireIntent | null>(null)
  const [stats, setStats] = useState<MarketStats>(initialStats || {})
  const [services, setServices] = useState<AgentService[]>((initialCatalog?.listings || []).map((listing) => listingToService(listing)))
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      fetch(catalogUrl(1, category, query, sort, readyOnly), { signal: controller.signal })
        .then(async (response) => {
          const data = await response.json()
  ...
  const loadMoreServices = async () => {
    ...
    setCatalogLoadMoreError(null)
    try {
      const response = await fetch(catalogUrl(nextPage, category, query, sort, readyOnly))
      const data = await response.json()
      if (!response.ok) throw new Error(data?.error || `Catalog request failed (${response.status})`)
      const fallback = Boolean(data.fallback)
    ...
    <main className={styles.page}>
      <header className={styles.hero}>
        ...
        <div className={styles.heroAside}>
          ...
          <div className={`${styles.heroStatus} ${catalogError || catalogIsFallback || paymentConfig?.new_payments_paused || !paymentServicesLive ? styles.heroStatusQuiet : ''}`}>
            <i />
            {catalogLoading ? 'Connecting to current catalog' : catalogError ? 'Catalog temporarily unavailable' : catalogIsFallback ? 'Preview mode — payments disabled' : paymentConfig?.new_payments_paused ? 'Payments temporarily paused' : paymentServicesLive ? 'Payment rails live · check seller readiness' : 'No payment rail available'}
          </div>
        </div>
      </header>
      ...
      <section className={styles.catalogSection}>
        ...
        <div className={styles.catalogHeader}>
          <div>
            ...
            <p>Listed services remain discoverable. Hire is enabled only when a seller has a valid payout address and an eligible payment rail is live.</p>
          </div>
        ...
        <div className={styles.catalogControls}>
          <label>
            ...
            <select value={readyOnly ? 'ready' : 'all'} onChange={(event) => setReadyOnly(event.target.value === 'ready')}>
              <option value="all">All listed services</option>
              <option value="ready">Hireable with payout wallet</option>
            </select>
          </label>
          <label>
            <span>SEARCH</span>
        ...
        <div className={styles.serviceGrid}>
          {filtered.map((service, index) => (
            <article className={styles.serviceCard} key={service.id}>
              <div className={styles.cardIndex}>{service.is_demo ? 'DEMO' : service.external_payment_ready ? 'PAYOUT READY' : 'PAYOUT PENDING'} / {String(index + 1).padStart(2, '0')}</div>

              <div className={styles.agentRow}>
                <div className={styles.avatar}>{service.initials}</div>
                <div className={styles.agentIdentity}>
                  <Link href={`/registry/${service.agent_id}`}>{service.agent_name}</Link>
                  <span>trust {publicTrustLabel(service.agent_trust, service.completed_trades, service.rating_count)} · {service.agent_trust_confidence} confidence<i style={{ background: trustColor(service.agent_trust) }} /></span>
                </div>
              ...
              <div className={styles.cardMetrics}>
                ...
                <span><i>TRUST</i>{publicTrustLabel(service.agent_trust, service.completed_trades, service.rating_count)}</span>
              </div>
        ...
        {!catalogLoading && !catalogError && filtered.length === 0 && <div className={styles.empty}>{readyOnly ? 'No payout-ready services match this search yet. Browse all listed services or ask a seller to configure a payout wallet.' : 'No listed services match this search yet.'}</div>}
      </section>
